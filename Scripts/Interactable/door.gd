...
## Tipo del lock della porta.
@export var unlock_type: MissionGraph.LockType = MissionGraph.LockType.FREE

## Estenzione laterale/superficiale della luce, rappresenta per quante tile si
## deve estendere la luce (es. 2 = due tile oltre la porta).
@export var side_extension: float = 2.0
## Estenzione laterale/superficiale della luce, rappresenta per quante tile si
## deve estendere la luce nella direzione secondaria (es. se la porta si estende
## verticalmente, allora lateralmente la luce si estende di mezza tile 0.5).
@export var minor_extension: float = 0.5
...
func _light_setup(size_px: Vector2) -> void:
	var center: Vector2 = size_px * 0.5
	point_light_2d.position = center
	
	var is_vertical: bool = tiles_y > tiles_x
	var target: Vector2 = size_px
	
	if is_vertical:
		target.x = size_px.x * (1.0 + 2.0 * side_extension)
		target.y = size_px.y * (1.0 + 2.0 * minor_extension)
	else:
		target.y = size_px.y * (1.0 + 2.0 * side_extension)
		target.x = size_px.x * (1.0 + 2.0 * minor_extension)
	
	if point_light_2d.texture != null:
		var texture_size: Vector2 = point_light_2d.texture.get_size()
		if !(texture_size.x > 0 and texture_size.y > 0): return
		
		point_light_2d.scale = Vector2(target.x / texture_size.x, target.y / texture_size.y)
	else:
		point_light_2d.scale = target / 256.0
