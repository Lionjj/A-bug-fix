...
func _add_decos(room: RoomTemplateMeta) -> void:
	...
	## TODO: Spegnere le luci di una stanza quando il giocatore sce fuori e 
	## migliorare le performance delle luci applicate ai nemici
	
	for type in Decoration.DECO_TYPE.values():
		deco_spawner.spawn_room_decos(room, deco_state, type)
