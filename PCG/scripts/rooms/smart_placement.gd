...
##
## Questo piazzamento inteligente viene usato per calcolare tutte le poszioni 
## utilizzabili all'interno di una stanza:
## - paviemnto;
## - soffitto;
## - pareti;
## - rientranze.
...
## Classe di supporto utilizzata per legare le poszioni indiviudate a un valore 
## rappresentsante l'importanza della suddetta poszione.[br]
...
## API: Dato una set di posizioni [param all_positions] e un numero di 
## posizioni minime da rispettare [param require_count] utilizza Voronoi 
## per individuare un sottoinsieme di punti "ben distribuiti".
...
## Funzione pubblica: che presa una [param tilemap]
...
## Funzione pubblica: restiuscie tutte le posizioni interne [param inner_space]
## della stanza [param room] che si trovano sul pavimento, se non c'è ne sono 
## restiusice un [Array] vuoto.
static func get_floor_air_cells(
...
## Funzione pubblica: Calcola le celle interne "aria" della stanza [param room] 
## tramite flood fill a partire da un punto di ingresso.[br]
## Aggiorna anche [member room.bounds] in coordinate global 
## (guarda anche [class RoomTemplateMeta]).
...
## Converte una cella [param cell] locale della [param tilemap] in posizione 
## world (centro cella), aggiungendo un offset opzionale [param offset].
...
## Funzione privata: genera e restitusice una lista di centoridi utilizzando 
## Voronoi a partire da una lista di poszioni: [param all_positions]. 
...
## Funzione privata: associa ad ogni centroide presente in [param centers] una 
## lista di posizioni [param all_positions].
...
## Funzione privata: seleziona un insieme di punti "esteticamente validi" 
## distribuiti tra più regioni [param regions].
##
## Assegna a ciascuna regione un numero di punti proporzionale
...
## Funzione privata (Helper): sceglie [param count] punti in [param region] 
## cercando una distribuzione più uniforme possibile.
## Strategia: primo punto vicino al centro [param center], poi massimizza la 
## distanza minima dai già selezionati.
...
## Funzione privata - Flood fill interno: parte da [param start] e visita solo 
## celle entro l'area usata della TileMap, fermandosi su celle solide. 
## Ritorna solo celle "aria" interne.
...
## Funzione privata: Trova un punto "vuoto" iniziale per il flood interno 
## [method _flood_internal_area].
## Strategia: scansiona dall'alto verso il basso cercando una cella senza tile.
...
## Funzione privata Calcola i bounds globali di un set di celle interne 
## [param internal_cells] di una [param tilemap].
...
## Funzione privata: Calcola il grado di "camminabilità" della cella [param cell] 
## locale visitando le celle adiacenti interne (4-dir) in [param inner_lookup].
static func _walkable_degree(
	cell: Vector2i, 
...
## Funzione privata: Ritorna true se la cella [param cell] è adiacente a un "muro" tra tutte le 
## celle possibili param [inner_lookup](vicino non è interno).
...
## Funzione pubblica: Valuta e ordina una lista di candidati [param candidates] 
## usando punteggi legati alla topologia interna della stanza [param room].
##
## - Evita un'area vicino all'entry per fairness ([param min_dist_from_entry]).
## - Applica score per: dead-end, corridoi, open area e vicinanza ai muri.
## - Oversampling: restituisce un set più ampio (circa [parma require_count] * 4)
## da cui poi scegli in modo deterministico/random.
...
## Funzione pubblica: Calcola celle in rientranze verticali nei muri partendo 
## dalle celle interne della stanza [param inner_space].
## "Nicchie": cella interna con muro a sinistra o a destra e con supporto sotto.
...
## Funzione privata: Ritorna true se la cella [param cell] fa parte di un 
## "pozzo" verticale (shaft): interno e chiuso da solidi su entrambi i lati. [br]
## - [param inner_space]: celle iterne della stanza.
static func _is_shaft_cell(cell: Vector2i, inner_space: Dictionary[Vector2i, bool]) -> bool:
	if !inner_space.has(cell): return false
	var left_solid: bool = !inner_space.has(cell + Vector2i.LEFT)
	var right_solid: bool = !inner_space.has(cell + Vector2i.RIGHT)
	return left_solid and right_solid

## Funzione privata: Calcola la profondità del pozzo partendo da [param cell] 
## andando verso il basso finché resta shaft.
## - [param inner_space]: celle iterne della stanza.
## - [param max_depth]: limite di sicurezza del controllo.
static func _shaft_depth_from(
	cell: Vector2i, 
	inner_space: Dictionary[Vector2i, bool], 
	max_depth: int = 64
) -> int:
	var depth: int = 0
	var cur: Vector2i = cell
	while depth < max_depth and _is_shaft_cell(cur, inner_space):
		depth += 1
		cur += Vector2i.DOWN
	return depth

## Funzione pubblica: Estrae celle target nei "pit" (pozzi) abbastanza profondi.
## Ritorna la cella vicino al fondo 
## (con offset configurabile [param bottom_offset]).
## - [param inner_space]: celle iterne della stanza.
## - [param min_delpth]: limite di sicurezza del controllo.
...
## Funzione pubblica: individua un set di posizioni utilizzabili dalla trappole
## tra le celle interne [param inner_space] della stazna [param room].
## - min_pit_depth: limite di sicurezza del controllo 
## (vedi [method get_pit_cells_local]);
...
## Fuznione pubblica: calcola un set di celle "aria" interne [param inner_space],
## con un tile solido sopra, per individuare le pozioni per le decorazioni 
## [member Decoration.DECO_TYPE.CEIL] della stanza [param room].
static func get_ceiling_air_cells(
	room: RoomTemplateMeta, 
	inner_space: Array[Vector2i]
) -> Array[Vector2i]:
	...
	var inner_lookup: Dictionary[Vector2i, bool] = {}
	for c in inner_space: inner_lookup[c] = true

	var out: Array[Vector2i] = []
	for cell in inner_space:
		var above : Vector2i = cell + Vector2i.UP
		var air_here : bool = tilemap.get_cell_tile_data(cell) == null
		var solid_above : bool = tilemap.get_cell_tile_data(above) != null
		if air_here and solid_above and inner_lookup.has(cell):
			out.append(cell)
...
## Fuznione pubblica: Ritorna le celle occupate orizzontalmente da un oggetto 
## con footprint [param footprint], centrato su [param center]. 
## Se footprint è pari, mette una cella in più a destra (deterministico).
static func get_footprint_cells(center: Vector2i, footprint: int) -> Array[Vector2i]:
	...
	# Numero di celle a sinistra e a destra rispetto al centro
	# Regola: se pari, metto una cella in più a destra (deterministico)
	var left_count := (footprint - 1) / 2          # floor
	var right_count := footprint - 1 - left_count  # resta
...
## Funzione pubblica: Filtra [param candidates] mantenendo solo quelli 
## piazzabili a terra con footprint orizzontale.
## Requisiti:
## - tutte le celle footprint [param footprint] devono essere interne 
## [param inner_lookup];
## - devono essere aria (nessun tile);
## - sotto ogni cella footprint deve esserci solido;
static func filter_candidates_by_footprint_ground(
	...
	var tilemap := room.collision
	var out: Array[Vector2i] = []
	if tilemap == null: return out

	for center in candidates:
		var cells := get_footprint_cells(center, footprint)
		var ok := true

		for c in cells:
			if not inner_lookup.has(c):
				ok = false
				break

			# devo piazzare in aria
			if tilemap.get_cell_tile_data(c) != null:
				ok = false
				break

			# sotto deve esserci solido
...
## Funzione pubblica: Filtra [param candidates] mantenendo solo quelli 
## piazzabili a soffitto con footprint orizzontale.
## Requisiti:
## - tutte le celle footprint[param footprint] devono essere interne 
## [param inner_lookup];
## - devono essere aria (nessun tile);
## - sopra ogni cella footprint deve esserci solido;
